...
find_optimal_model <- function(ed_shared_list,ec50_idv_full,dilutions_vector) {
  ...
  # True titers from individual model on full dilution series
  true <- ec50_idv_full
  ...
  for(n in reduced_points) {
    ...
    # Calculate MSE for each dilution combination (skip combination with NAs)
...
# Gold standard was extracted from individual model on full dilution series
...
# Calculate ratio of predicted titer to true titer
optimal_logtiters[cobovax_names] <- lapply(optimal_logtiters[cobovax_names], function(x) {
  x[, paste0("ratio_n", 2:5)] <- x[, paste0("logtiter_n", 2:5)] - x$logtiter_true
  return(x)
})
optimal_logtiters$pattinson[paste0("ratio_n", 2:5)] <- optimal_logtiters$pattinson[, paste0("logtiter_n", 2:5)] - optimal_logtiters$pattinson$logtiter_true 
